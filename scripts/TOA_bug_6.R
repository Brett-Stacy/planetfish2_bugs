## 30 June 19 - R 3.5.3 - earthfish version 0.1.0

## Let's find that bug!
## Different Species - Made up LHP with straight line growth
...
# Ages
TOA_max_age = 35 # made up

# Growth. Von Bertalanfy.
TOA_L_inf = 2080 # made up
TOA_K     = 0.01 # made up
TOA_t_0   = -1.0  # made up
TOA_CV    = 0.122   # made up
...
LL_sel = list(top=9, sigma_left=3, sigma_right=10)
x = 1:TOA_max_age
y = calc_VBlen(1:35, x, c(TOA_L_inf, TOA_K, TOA_t_0, TOA_CV))
z = calc_VBweight(1:35, x, c(TOA_L_inf, TOA_K, TOA_t_0, TOA_CV), list(a = TOA_wl_c, b = TOA_wl_d))
m = ogive(TOA_maturity_ogive, x, list(x50 = TOA_a_50, x95 = TOA_a_95))
s = ogive("double_normal", x, LL_sel)
par(mfrow = c(2,2))
plot(x, y, type = "l", main = "Fictional VB Length", xlab = "Age", ylab = "Length")
plot(x, z, type = "l", main = "Fictional VB Weight", xlab = "Age", ylab = "Weight")
plot(x, m, type = "l", main = "Fictional Maturity", xlab = "Age", ylab = "Probability Mature")
plot(x, s, type = "l", main = "Fictional Selectivity", xlab = "Age", ylab = "Probability Selected")
...
LL_sel <- list(top=9, sigma_left=3, sigma_right=10) # made up
...
temp1 = read.csv("TOA_bug_6_Niter_1000.csv")
...
plot_ts_uncertainty(d = true_ssb1/1000, d2 = est_ssb1/1000,
                  ylab = "SSB ('000 Tonnes)", xlab = "Year", main = "Fictional")
